# lists sorted but...then also if
# someone exits and his name comes alphabetically earlier to the other one it will say hello for both
import cv2
import os
    ...
    def classify_face(self, img, face_locations):
        ...
        if len(face_names) != 0:
            if face_names != self.old_names:
                for (i, w) in itertools.zip_longest(face_names, self.old_names):
                    if i != w and i is not None:
                        self.speak("Hello " + i)
                self.old_names = face_names
        elif len(face_names) == 0:  # problem here after exit the condition should run not running
            self.old_names = []
